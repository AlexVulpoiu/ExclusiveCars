...
export default class AddRentalAnnouncement extends Component {
    ...
    componentDidMount() {
        document.title = "Adaugă anunț de închiriere"
        ...
        fetch(`http://localhost:8090/api/organisations/myOrganisation`, {
            ...
            .then((data) => {
                const rentalCenterIds = [];
                for(let i in data["auto_services"]) {
                    rentalCenterIds.push(data["auto_services"][i]["id"]);
                }
                this.setState({myRentalCenters: rentalCenterIds});
            })
    ...
    userHasAccess(user) {
        return user !== null && user.roles.includes("ROLE_ORGANISATION")
            && this.state.myRentalCenters.includes(Number(this.props.match.params.rentalCenterId));
    }
    ...
    async handleSubmit(e) {
        ...
        if(this.checkBtn.context._errors.length === 0) {
            ...
            let carId = 0;
            await axios.post(`http://localhost:8090/api/rentalAnnouncements/add/${this.props.match.params.rentalCenterId}`, carDto, {
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
                    Authorization: authHeader().Authorization
                ...
                .catch((error) => {
                    this.props.history.push(`/rentalAnnouncements/add/${this.props.match.params.rentalCenterId}`);
                    window.location.reload();

                    sessionStorage.setItem("rentalAnnouncementAddMessage", error.response.data);
            ...
            try {
                ...
                this.props.history.push("/rentalAnnouncements/add");
                window.location.reload();
            }
        } else {
            this.setState({
    ...
    hideAlert() {
        ...
        sessionStorage.setItem("rentalAnnouncementAddMessage", "");
    }
    ...
    render() {
        ...
        if(this.state.myRentalCenters !== null && !this.userHasAccess(this.currentUser)) {
            setTimeout(() => {
        ...
        return (
            <div className={"col-md-12"}>
                <div>
                    {sessionStorage.getItem("rentalAnnouncementAddMessage") !== null && sessionStorage.getItem("rentalAnnouncementAddMessage") !== "" && (
                        <div
                            id={"notification"}
                            role="alert"
                        ...
                        >
                            ...
                            {sessionStorage.getItem("rentalAnnouncementAddMessage")}
                        </div>
                    )}
