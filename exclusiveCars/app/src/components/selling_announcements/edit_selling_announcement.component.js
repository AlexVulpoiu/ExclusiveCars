...
export default class EditSellingAnnouncement extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            negotiable: sessionStorage.getItem("carNegotiableEdit"),
            location: sessionStorage.getItem("carLocationEdit"),
            description: sessionStorage.getItem("carDescriptionEdit"),
            loading: true,
            // upload
        ...
        sessionStorage.setItem("carNegotiableEdit", "");
        sessionStorage.setItem("carLocationEdit", "");
        sessionStorage.setItem("carDescriptionEdit", "");
    }

    componentDidMount() {
        document.title = "Editează anunțul de vânzare"
        ...
        fetch(`http://localhost:8090/api/sellingAnnouncements/${this.props.match.params.id}`, {
            ...
            .then((data) => {
                this.setState({sellingAnnouncement: data, images: data["car"]["images"]});
                ...
                if(this.state.negotiable === null || this.state.negotiable === "") {
                    this.setState({negotiable: data["negotiable"] ? "DA" : "NU"});
                }
    ...
    isOwner(user, announcement) {
        return user !== null && user["id"] === announcement["user"]["id"];
    }
    ...
    async handleSubmit(e) {
        ...
        if(this.checkBtn.context._errors.length === 0) {
            ...
            let carId = 0;
            await axios.put(`http://localhost:8090/api/sellingAnnouncements/edit/${this.props.match.params.id}`, newSellingAnnouncement, {
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
                    Authorization: authHeader().Authorization
                ...
                .catch((error) => {
                    this.props.history.push(`/sellingAnnouncements/edit/${this.props.match.params.id}`);
                    window.location.reload();

                    sessionStorage.setItem("sellingAnnouncementEditMessage", error.response.data);
            ...
            try {
                ...
                for(let i in this.state.deletedImages) {
                    await this.removeImage(this.state.deletedImages[i], this.state.sellingAnnouncement["car"]["id"]);
                }
    ...
    hideAlert() {
        ...
        sessionStorage.setItem("sellingAnnouncementEditMessage", "");
    }
    ...
    render() {
        ...
        if (!this.isOwner(this.currentUser, sellingAnnouncement)) {
            setTimeout(() => {
        ...
        return (
            <div className={"col-md-12"}>
                <div>
                    {sessionStorage.getItem("sellingAnnouncementEditMessage") !== null && sessionStorage.getItem("sellingAnnouncementEditMessage") !== "" && (
                        <div
                            id={"notification"}
                            role="alert"
                        ...
                        >
                            ...
                            {sessionStorage.getItem("sellingAnnouncementEditMessage")}
                        </div>
                    )}
