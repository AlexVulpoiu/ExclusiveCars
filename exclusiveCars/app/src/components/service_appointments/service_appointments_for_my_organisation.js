...
export default class ServiceAppointmentsForMyOrganisation extends Component {
    ...
    componentDidMount() {
        ...
        fetch(`http://localhost:8090/api/serviceAppointments/forMyOrganisation`, {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                Authorization: authHeader().Authorization
    ...
    render() {
        ...
        if(serviceAppointments === [] || serviceAppointments === null) {
            return (
                <div className={"col-md-12"}>
                    <h2>Nu există nicio programare pentru service-urile din această organizație!</h2>
                </div>
            );
        }
        ...
        return (
            <>
                ...
                <div style={{height: "50px"}}>
                    <h1 style={{float: "left"}}>Programări <a href={`/organisations/myOrganisation`} target={"_blank"}>{this.state.organisation["name"]}</a></h1>

                    <Button color={"success"} tag={Link} to={"/myAutoServices"} style={{float: "right"}}>
                        Service-urile mele
                    </Button>
                </div>
                <br/>
